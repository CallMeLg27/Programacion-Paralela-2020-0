...
int main(int argc, char * argv[]) {
    ...
    if(my_rank==0){
		...
		cin>>c;
        //verificamos que el numero de filas sea multiplo del numero de procesadores,
        //de lo contrario, lo convertimos a un multiplo del numero de procesadores
		if (f < rank_sz)
        	f = rank_sz;
        else {
            int i = 1, num = rank_sz;
            ...
            if (f != num) {
                cout << "Cantidad de filas de la matriz cambiada a " << num << endl;
                f = num;
            }
        }
	}
	//enviamos el numero de filas a todos los procesos
	MPI_Bcast(&f, 1, MPI_INT, 0, comm);
	MPI_Bcast(&c, 1, MPI_INT, 0, comm);
	
    A = new long *[f]; // Reservamos "f" filas
    x = new long [f]; // El vector sera del mismo tamanio que el numero
    ...
    MPI_Scatter(A[0], // Matriz que vamos a compartir
            c*f/rank_sz, // Numero de columnas a compartir
    ...
    MPI_Bcast(x, // Dato a compartir
            f, // Numero de elementos que se van a enviar y recibir
